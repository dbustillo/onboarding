/*
  # Fix Task Template Display

  1. Changes
    - Create a new default template with proper structure
    - Ensure template is properly formatted for display in admin interface
    - Fix any issues with task data structure
    
  2. Purpose
    - Make sure templates appear in the admin interface
    - Ensure tasks can be assigned to clients
    - Fix any data format issues preventing proper display
*/
...
BEGIN
  SELECT COUNT(*) INTO template_count FROM onboarding_templates;
  
  -- Only create a new template if none exist
  IF template_count = 0 THEN
    ...
    RAISE NOTICE 'Created default onboarding template';
  ELSE
    RAISE NOTICE 'Templates already exist, skipping creation';
  END IF;
END $$;